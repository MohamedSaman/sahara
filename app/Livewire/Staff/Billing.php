        ...
        if ($this->paymentType == 'full') {
            ...
            if ($this->paymentMethod == 'cheque') {
                if (empty($this->paymentReceiptImage)) {
                    ...
                    return;
                } elseif ($this->paymentReceiptImage && $this->paymentReceiptImage->getSize() > 2048 * 2048) {
                    $this->js('swal.fire("Validation Error", "Receipt size must be less than 4MB", "error")');
                    return;
            ...
            if ($this->initialPaymentAmount > 0) {
                ...
                if ($this->initialPaymentMethod == 'cheque') {
                    if (empty($this->initialPaymentReceiptImage)) {
                        $this->js('swal.fire("Validation Error", "Payment Receipt is required", "error")');
                        return;
                    } elseif ($this->initialPaymentReceiptImage && $this->initialPaymentReceiptImage->getSize() > 2048 * 2048) { // Change from 1024*1024 to 2048*2048
                        $this->js('swal.fire("Validation Error", "Receipt size must be less than 4MB", "error")');
                        return;
                    ...
                    if (empty($this->initialPaymentReceiptImage)) {
                        $this->js('swal.fire("Validation Error", "Payment Receipt is required", "error")');
                        return;
                    } elseif ($this->initialPaymentReceiptImage && $this->initialPaymentReceiptImage->getSize() > 2048 * 2048) { // Change from 1024*1024 to 2048*1024
                        $this->js('swal.fire("Validation Error", "Receipt size must be less than 4MB", "error")');
                        return;
                    }
                }
            ...
            if ($this->balanceAmount > 0) {
                ...
                if ($this->balancePaymentMethod == 'cheque') {
                    if ($this->balancePaymentReceiptImage){
                        ...
                        return;
                    } elseif ($this->balancePaymentReceiptImage && $this->balancePaymentReceiptImage->getSize() > 2048 * 2048) {
                        $this->js('swal.fire("Validation Error", "Receipt size must be less than 4MB", "error")');
                        return;
                    }
