    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body p-4" id="payslipContent">
                        ...
                        <div class="row mb-3">
                            <div class="col-12">
                                ...
                                <table class="table table-sm table-bordered">
                                    <tbody>
                                        <tr><td>Basic Salary</td><td class="text-end">Rs.{{ number_format($selectedSalary->basic_salary - $selectedSalary->allowance ?? 0, 2) }}</td></tr>
                                        <tr><td>Allowance</td><td class="text-end">Rs.{{ number_format($selectedSalary->allowance ?? 0, 2) }}</td></tr>
                                        <tr><td>Bonus</td><td class="text-end">Rs.{{ number_format($selectedSalary->bonus ?? 0, 2) }}</td></tr>
                                        <tr><td>Overtime</td><td class="text-end">Rs.{{ number_format($selectedSalary->overtime ?? 0, 2) }}</td></tr>
                                        <tr><td>Gross Earnings</td><td class="text-end">Rs.{{ number_format($grossEarnings, 2) }}</td></tr>
                                        <tr><td>Loan Repayment</td><td class="text-end">Rs.{{ number_format($selectedSalary->loan_deduction ?? 0, 2) }}</td></tr>
                                        <tr><td>Other Deductions</td><td class="text-end">Rs.{{ number_format($selectedSalary->deductions ?? 0, 2) }}</td></tr>
                                        <tr><td>Total Deductions</td><td class="text-end text-danger">(Rs.{{ number_format($totalDeductions, 2) }})</td></tr>
                                        <tr class="table-success fw-bold"><td>Net Salary Payable</td><td class="text-end">Rs.{{ number_format($selectedSalary->net_salary ?? 0, 2) }}</td></tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                        <div class="alert alert-success text-center mt-3" role="alert">
                            <h5 class="alert-heading fw-bolder">NET SALARY PAYABLE</h5><hr>
                ...
                @else
                    <div class="text-center p-5"><p class="text-muted">Loading payslip information...</p></div>
                @endif
